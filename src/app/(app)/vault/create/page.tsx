    ...
    <div className="mx-auto max-w-5xl space-y-8">
      <form onSubmit={handleCreate} className="ny-panel space-y-8 p-6 md:p-8">
        <StepWizard steps={steps} currentStep={currentStep} onStepChange={setCurrentStep} showNavigation={false}>
          {currentStep === 0 && (
            <div className="space-y-7">
              <div className="grid gap-4 md:grid-cols-[1fr_1.1fr]">
                <div>
                  <label className="mb-1 block text-xs font-medium text-zinc-300">Vault name</label>
                  <input
                    value={vaultName}
                    onChange={(event) => setVaultName(event.target.value)}
                    className="ny-input w-full px-3 py-2 text-white"
                    placeholder="e.g. Main MetaMask wallet"
                    maxLength={80}
                  />
                  <p className="mt-1 text-xs text-zinc-500">Required. Use a name your family or team will recognize.</p>
                </div>
                <div>
                  ...
                  <input
                    ...
                    maxLength={120}
                  />
                  <p className="mt-1 text-xs text-zinc-500">{120 - vaultDescription.length} characters left.</p>
                </div>
              </div>

              <div>
                <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                  <p className="text-xs font-medium uppercase tracking-[0.16em] text-zinc-400">Tags</p>
                  <p className="text-xs text-zinc-500">{tags.length}/5 selected</p>
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-full border px-3 py-1 text-xs transition disabled:opacity-40 ${selected
                          ...
                          : 'border-white/10 bg-white/[0.03] text-zinc-400 hover:border-amber-200/35 hover:text-amber-100'
                        }`}
                      >
                        {tag}
                      </button>
                ...
                <div className="mt-3 flex gap-2">
                  <input
                    ...
                    className="ny-input min-w-0 flex-1 px-3 py-2 text-sm text-white"
              ...
              <div>
                <p className="ny-label text-amber-200">What are you saving?</p>
              ...
              <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                {secretType === 'seed' && (
                  <div className="space-y-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <h3 className="font-display text-xl text-white">Enter seed phrase</h3>
                        <p className="mt-1 text-sm text-zinc-400">Type each word in order. Nythera will join them before encryption.</p>
                      </div>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                      {Array.from({ length: seedWordCount }).map((_, index) => (
                        <label key={index} className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2">
                ...
                {secretType === 'private-key' && (
                  <div className="space-y-3">
                    <h3 className="font-display text-xl text-white">Enter private key</h3>
                    <div className="flex gap-2">
                      <input
                        value={privateKey}
                        onChange={(event) => setPrivateKey(event.target.value)}
                        type={showPrivateKey ? 'text' : 'password'}
                        className="ny-input min-w-0 flex-1 px-3 py-2 font-mono text-sm text-white"
                        placeholder="0x..."
                        autoComplete="off"
                        spellCheck={false}
                      />
                    ...
                    <p className={`text-xs ${privateKey && !isPrivateKey(privateKey) ? 'text-amber-200' : 'text-zinc-500'}`}>
                ...
                {secretType === 'password' && (
                  <div className="space-y-4">
                    <h3 className="font-display text-xl text-white">Save a password or PIN</h3>
                    <div className="grid gap-3 md:grid-cols-2">
                      <input
                        value={password}
                        onChange={(event) => setPassword(event.target.value)}
                        type={showPassword ? 'text' : 'password'}
                        className="ny-input px-3 py-2 text-white"
                    ...
                    {password && confirmPassword && password !== confirmPassword && (
                      <p className="text-xs text-amber-200">The two fields do not match yet.</p>
                    )}
                  </div>
                )}
                ...
                {secretType === 'note' && (
                  <div className="space-y-3">
                    <h3 className="font-display text-xl text-white">Write a custom note</h3>
                    <textarea
                      rows={7}
                      value={customNote}
                      onChange={(event) => setCustomNote(event.target.value)}
                      className="ny-input w-full px-3 py-3 text-sm leading-6 text-white"
                ...
                {secretType === 'file' && (
                  <div className="space-y-4">
                    <h3 className="font-display text-xl text-white">Upload an encrypted file backup</h3>
                    <label
                      onDragOver={(event) => {
                        event.preventDefault();
                      ...
                      className={`block cursor-pointer rounded-2xl border border-dashed p-6 text-center transition ${draggingFile
                        ...
                        : 'border-white/15 bg-white/[0.03] hover:border-amber-200/35'
                      }`}
                    >
                      <input
                        type="file"
                        ...
                        className="sr-only"
                      />
                      <p className="font-medium text-white">{file ? file.name : 'Drop a file here or click to choose'}</p>
                    ...
                    <div className={`rounded-xl border p-3 text-sm ${hasEnoughStorageCredits
                      ...
                      {storageCredits.error && (
                        <p className="mt-2 text-xs text-amber-100">{storageCredits.error}</p>
                      )}
                    </div>
                    <div>
                      <label className="mb-1 block text-xs font-medium text-zinc-300">Optional file note</label>
                      <textarea
                        rows={2}
                        value={fileNote}
          ...
          {currentStep === 1 && (
            <div className="space-y-4">
              <p className="ny-label text-amber-200">Trusted contacts</p>
              <h2 className="font-display text-xl text-white">Who should be able to recover this?</h2>
              ...
              <textarea
                ...
                className="ny-input w-full px-3 py-3 text-sm text-white"
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              <p className="ny-label text-amber-200">Safety</p>
              <h2 className="font-display text-xl text-white">How this vault will be protected</h2>
              <div className="grid gap-4 md:grid-cols-2">
                <div className="ny-tile p-4">
                  <p className="ny-label">Recovery access</p>
                  <p className="mt-2 text-sm text-zinc-300">Only you and trusted contacts can open this vault.</p>
                </div>
                <div className="ny-tile p-4">
                ...
                <div className="ny-tile p-4">
                  ...
                  <p className="mt-2 text-sm text-zinc-300">Encrypted in your browser before saving.</p>
                </div>
                <div className="ny-tile p-4">
              ...
              {secretType === 'file' && file && (
                <div className="ny-tile p-4">
                  <label className="mb-2 block text-xs font-medium text-zinc-300">File storage reminders</label>
                  <select
                    value={renewalMode}
                    onChange={(event) => setRenewalMode(event.target.value as RenewalMode)}
                    className="select-chevron ny-input w-full px-3 py-2 text-white"
                  >
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              <p className="ny-label text-amber-200">Finish</p>
              <h2 className="font-display text-xl text-white">Review before saving</h2>
              <div className="grid gap-4 md:grid-cols-2">
        ...
        {loading && progressStage !== 'idle' && (
          <div className={`rounded-xl border p-4 text-sm text-zinc-200 ${progressStage === 'complete'
            ? 'ny-success-surface'
            ...
            {progressStage === 'complete' ? (
              ...
              <>
                <p className="text-xs font-medium uppercase tracking-[0.16em] text-amber-100">Creating vault</p>
                <p className="mt-2 text-zinc-300">{createProgressCopy[progressStage]}</p>
              </>
            )}
          </div>
        )}
        ...
        {error && (
          <div className="rounded-xl border border-red-300/30 bg-red-300/10 p-4 text-sm">
            <p className="text-xs font-medium uppercase tracking-[0.16em] text-red-200">Could not create vault</p>
            <p className="mt-2 text-white/80">{formattedError.summary}</p>
            {formattedError.details && (
              <details className="mt-3">
                <summary className="cursor-pointer text-xs text-zinc-400">Advanced details</summary>
                <p className="mt-2 break-words font-mono text-xs text-zinc-500">{formattedError.details}</p>
              </details>
            )}
          </div>
        )}
  ...
  return (
    <button
      ...
      className={`min-h-40 rounded-2xl border p-4 text-left transition ${selected
        ? 'border-amber-200/50 bg-amber-300/10 shadow-[0_0_30px_rgba(217,180,95,0.12)]'
