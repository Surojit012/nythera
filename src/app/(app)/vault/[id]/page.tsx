  ...
  if (!vault) {
    return (
      <div className="max-w-3xl mx-auto">
        ...
        <Link href="/dashboard" className="mt-4 inline-block font-mono text-sm text-violet-200">
          Back to dashboard
        </Link>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <section className="ny-panel p-6 md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="ny-label text-amber-200">Vault details</p>
            <h1 className="font-display text-3xl md:text-4xl text-white mt-2">
              {vaultName}
            </h1>
            ...
            {(vault.tags ?? []).length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {(vault.tags ?? []).map((tag) => (
                  <span key={tag} className="rounded-full border border-amber-200/20 bg-amber-300/10 px-2.5 py-1 text-xs text-amber-100">
                    {tag}
                  </span>
                ))}
              </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <div className="ny-tile p-4">
            ...
            <p className="text-sm text-zinc-300 mt-2">{vaultName}</p>
          </div>
          <div className="ny-tile p-4">
            <p className="ny-label">Description</p>
            <p className="text-sm text-zinc-300 mt-2">{vault.description?.trim() || 'No description added'}</p>
          </div>
          <div className="ny-tile p-4">
            <p className="ny-label">Created</p>
            <p className="text-sm text-zinc-300 mt-2">{new Date(vault.createdAt).toLocaleString()}</p>
          </div>
          <div className="ny-tile p-4">
          ...
          {vault.cdr ? (
            <div className="ny-tile p-4">
              ...
              <p className="text-sm text-zinc-300 mt-2">{vault.cdr.uuid}</p>
            ...
            <div className="ny-tile p-4">
              ...
              <p className="text-sm text-zinc-300 mt-2">Legacy local vault (AES + Shamir)</p>
            </div>
          )}
          <div className="ny-tile p-4">
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="ny-panel p-6">
          ...
          <div className="mt-4 space-y-4 text-sm text-zinc-400">
            <div>
              <p className="text-white">Vault created</p>
              <p className="text-xs text-zinc-500">{new Date(vault.createdAt).toLocaleString()}</p>
            </div>
            {vault.cdr && (
              <details className="rounded-xl border border-white/10 bg-black/20 p-3">
                <summary className="cursor-pointer text-sm text-zinc-300">Advanced blockchain details</summary>
                <div className="mt-3 space-y-3">
                  <div>
                    <p className="text-white">Storage reservation</p>
                    <p className="text-xs text-zinc-500 break-all">{vault.cdr.allocateTxHash}</p>
                  </div>
                  <div>
                    <p className="text-white">Encrypted save</p>
                    <p className="text-xs text-zinc-500 break-all">{vault.cdr.writeTxHash}</p>
                  </div>
                </div>
              </details>
            )}
        ...
        <div className="ny-panel p-6">
          ...
          <div className="mt-4 space-y-3 text-sm text-zinc-400">
            <div className="flex items-center justify-between">
              <span>Status</span>
              <span className="font-mono text-white/80">{vault.status}</span>
            </div>
            <div className="flex items-center justify-between">
              <span>Guardians</span>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-white/80">Verified</span>
            </div>
          </div>
        </div>
        ...
        <div className="ny-panel p-6">
          ...
          <p className="font-display text-2xl text-white mt-3">{recoveryEvents.length}</p>
          <p className="text-xs text-zinc-500 mt-1">
            {recoveryEvents.length === 1 ? 'Successful recovery' : 'Successful recoveries'}
          </p>
          {recoveryEvents.length === 0 ? (
            <p className="text-sm text-zinc-400 mt-4">No recovery events recorded.</p>
          ) : (
              ...
              {recoveryEvents.slice(0, 3).map((event) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <span className="text-sm text-white">Vault recovered</span>
                    <span className="font-mono text-[11px] text-zinc-500">
                      {event.contentType === 'walrus-file' ? 'File' : 'Secret'}
                    </span>
                  </div>
                  <p className="text-xs text-zinc-500 mt-1">{new Date(event.recoveredAt).toLocaleString()}</p>
      ...
      {vault.cdr && (
        <section className="ny-panel p-6 md:p-8 space-y-4">
          <div>
            ...
            <h2 className="font-display text-xl text-white mt-2">Edit guardians</h2>
          ...
          {recipientEntries.length > 0 && (
            <div className="space-y-2">
              <p className="font-mono text-xs text-zinc-400">Current guardians (click remove):</p>
              <div className="flex flex-wrap gap-2">
                {recipientEntries.map((entry) => (
                  <button
                    ...
                    className="border border-white/10 bg-black/30 px-2.5 py-1 font-mono text-xs text-zinc-300 hover:border-red-300/50 hover:text-red-300"
                    title={`Remove ${entry}`}
                  >
                    {entry} x
                  </button>
      ...
      {confirmingDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="ny-modal relative z-10 w-full max-w-md p-6">
            ...
            <h3 className="mt-2 font-display text-3xl leading-none text-white">Delete this vault?</h3>
            ...
            <div className="mt-4 ny-tile p-3">
              <p className="text-sm text-zinc-300">{vaultName}</p>
              {vault.cdr?.uuid && (
                <details className="mt-2">
                  <summary className="cursor-pointer text-xs text-zinc-500">Advanced details</summary>
                  <p className="text-xs text-zinc-600 mt-1">Vault ID {vault.cdr.uuid}</p>
                </details>
              )}
            </div>
            <div className="mt-4">
              <label className="block text-xs font-mono text-zinc-400 mb-1">Type DELETE to confirm</label>
              <input
                value={deleteConfirmText}
                onChange={(event) => setDeleteConfirmText(event.target.value)}
                className="w-full ny-input px-3 py-2 text-sm text-white font-mono"
                placeholder="DELETE"
                autoComplete="off"
              />
            </div>
            {signatureChecksum && (
              <div className="ny-success-surface mt-4 inline-flex items-center gap-2 rounded-lg border px-3 py-2">
                ...
                <span className="text-[11px] font-mono text-zinc-400">{signatureChecksum}</span>
              </div>
            )}
            {deleteError && <p className="text-sm text-red-300 mt-3">{deleteError}</p>}
