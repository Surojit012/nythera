...
/// @title AccessConditionV2
/// @notice CDR condition that reads initial access from conditionData and supports creator-managed overrides.
contract AccessConditionV2 {
    ...
    function setAccessOverride(bytes calldata conditionData, address account, bool allowed) external {
        ...
        bytes32 accessKey = keccak256(conditionData);
        accessOverrides[accessKey][account] = AccessOverride({ set: true, allowed: allowed });
        emit AccessOverrideSet(accessKey, account, allowed);
    }
